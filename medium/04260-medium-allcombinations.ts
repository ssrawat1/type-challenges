...
type RemoveFirst<S extends string, C extends string> =
  S extends `${infer L}${C}${infer R}` ? `${L}${R}` : S

type AllCombinations<S extends string, C extends string = StrToUnion<S>> =
  | ''
  | (C extends string ? `${C}${AllCombinations<RemoveFirst<S, C>>}` : never)
