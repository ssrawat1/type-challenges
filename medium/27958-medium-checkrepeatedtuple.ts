...
type Check<F, T> = T extends [infer F1, ...infer R] ? IsEqual<F, F1> extends true ? true : Check<F, R> : false

type CheckRepeatedTuple<T extends unknown[]> = T extends [infer F, ...infer R] ? Check<F, R> extends true
  ? true : CheckRepeatedTuple<R> : false
